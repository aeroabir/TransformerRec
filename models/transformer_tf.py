...
def transformer_layer(
    ...
    in1 = Input(shape=(seq_len,))
    # enc_padding_mask = create_padding_mask(in1)
    enc_padding_mask = None
    encoder = Encoder(
        num_layers, d_model, num_heads, dff, vocab_size, seq_len, pe_input, rate
    )
    encoded = encoder(in1, training, enc_padding_mask)
    return in1, encoded


def build_multilevel_transformer_unequal(
    inp_seq_len, tgt_seq_len, input_vocab_size, target_vocab_size, **kwargs
):
    """
    ...
    pe_input = kwargs.get("pe_input", 1000)
    pe_target = kwargs.get("pe_target", 1000)
    training = kwargs.get("training", True)
    decoder_type = kwargs.get("decoder_type", "transformer")
    ...
    dec_in = Input(shape=(tgt_seq_len,))
    inputs.append(dec_in)
    ...
    merge = Dense(d_model, activation="relu")(merge)  # (b, inp_seq_len, h)

    if decoder_type == "lstm":
        merge = Flatten()(merge)
        state_h = Dense(d_model, activation="relu")(merge)
        state_c = Dense(d_model, activation="relu")(merge)
        encoder_states = [state_h, state_c]

        embedding = tf.keras.layers.Embedding(target_vocab_size, d_model)
        dec_in = embedding(dec_in)

        decoder_lstm = LSTM(d_model, activation="tanh", return_sequences=True)
        output = decoder_lstm(dec_in, initial_state=encoder_states)
        output = Dense(target_vocab_size, activation="linear")(output)
        # output = TimeDistributed(Dense(target_vocab_size, activation="linear"))(
        #     lstm_output
        # )
        ...
        dec_padding_mask = create_padding_mask(merge)
        # dec_padding_mask = None
        look_ahead_mask = create_look_ahead_mask(tgt_seq_len)
        dec_target_padding_mask = create_padding_mask(dec_in)
        look_ahead_mask = tf.maximum(dec_target_padding_mask, look_ahead_mask)

        # dec_output, _ = decoder(dec_in, merge, training, None, None)
        dec_output, _ = decoder(
            dec_in, merge, training, look_ahead_mask, dec_padding_mask
        )
        output = Dense(target_vocab_size)(dec_output)
